...
export default function MedicationsPanel({ selectedFamilyId = null }) {
  ...
  const openEdit = (med) => {
    ...
    setForm({
      ...
      schedules: Array.isArray(med.schedules) ? med.schedules.join(', ') : '',
    })
    setEditId(med.id)
    setShowModal(true)
  }
            ...
            return (
              <div key={med.id} style={{
                ...
                <div style={{ position: 'absolute', top: '1.25rem', right: '1.25rem', display: 'flex', gap: '0.35rem' }}>
                  <button onClick={() => openEdit(med)} style={{ background: 'white', border: '1px solid var(--color-surface-200)', padding: '0.3rem', borderRadius: '6px', cursor: 'pointer', color: 'var(--color-surface-500)' }} title="Editar">
                    <FaEdit size={12} />
                  </button>
                  <button onClick={() => handleDelete(med.id, med.name)} style={{ background: 'white', border: '1px solid #fecaca', padding: '0.3rem', borderRadius: '6px', cursor: 'pointer', color: '#ef4444' }} title="Eliminar">
                    <FaTrash size={12} />
                  </button>
                </div>
          ...
          <div style={{
            ...
            <form onSubmit={handleSave} style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem', overflowY: 'auto', flex: 1 }}>
              ...
              <div>
                ...
                <input
                  type="text" placeholder="Ej: 08:00, 20:00" value={form.schedules}
